...
\begin{itemize}
\item Chapter \ref{sec:orge9b9d99} shows how coinductive types can be defined.
Here, we will define the stream and function type, as well as some
functions on the stream.

\item We will see in Chapter \ref{sec:orgdf2cbe6} how
...
\item In Chapter \ref{sec:org8e5771f} we
...
\item We will then in Chapter \ref{sec:org6e38a45} explain how this theory is
...
In Section \ref{sec:org1a8da02} we will look at the implementation in Coq.
...
types. In Section \ref{sec:org182dbb5} we look at the implementation in
...
\label{sec:org182dbb5}
In Agda coinductive types were first also introduced as positive types. In
Section \ref{sec:org8c80141} we will look at them in
detail. In Section \ref{sec:orgff39744} we describe the correct
...
types. They are described in Section \ref{sec:org9ed665f}.

\subsection{Positive Coinductive Types in Agda}
\label{sec:org8c80141}
...
natural numbers with one additional element, infinity. See \ref{sec:orgd0bc9d9}
...
We will give the rules for the theory in Section \ref{sec:orgeaeda0d} and a detailed
explanation of the reduction in \ref{sec:org11a9f43}.

\chapter{Implementation}
\label{sec:org6e38a45}
...
in Section \ref{sec:orgeaeda0d}.

In Section \ref{sec:org86fd169} we will develop the abstract syntax of our language
from the raw syntax in the paper. Then, we rewrite the typing rules in \ref{sec:orgeaeda0d}. At last we look at the implementation of the reduction in \ref{sec:org11a9f43}
\section{Abstract Syntax}
\label{sec:org86fd169}
...
will describe declarations in Section \ref{sec:org2ecf649}. We will also be able to
bind expressions to names. In Section \ref{sec:org646636c} we will define the syntax
...
\ref{sec:orgb81c16c}, we will see how the examples from the paper look in our concrete
syntax.
\subsection{Declarations}
\label{sec:org2ecf649}
...
describe in Section \ref{sec:orgf47c22d}. $\mathtt{GlobalExprVar}$\;refers to variables from
...
\DisplayProof
\end{center}
These are structural the same rules like this for \textbf{TyCtx}.  The difference is that \textbf{ParCtx}
...
\DisplayProof
\end{center}
\(\longrightarrow_T\) is defined in Section \ref{sec:org11a9f43}.
...
\subsection{Unit type and expression introduction}
\label{sec:orgaa9506f}
...
\section{Evaluation}
\label{sec:org11a9f43}
There are two kinds of reduction steps in this system. The implementation of
this is in $\mathtt{Eval.hs}$. Will give the formal definition in the following.

The first is a reduction on the type level (written $\mathtt{\longrightarrow}$). It is defined
as follows:
\begin{align*}
...
We will show the proof in appendix \ref{sec:org7541cdb}.
...
it as a coinductive type $\mathtt{Terminal}$\;with no destructors. It gets
...
of type $\mathtt{Intial}$, we could generate with $\mathtt{exfalsum}$\;a value of arbitrary type
$\mathtt{C}$.
\begin{lstlisting}
...
as an inductive type.  It is either $\mathtt{A}$\;or $\mathtt{B}$.  We have one constructor $\mathtt{Left}$
for $\mathtt{A}$\;and one constructor Right for $\mathtt{B}$.
\begin{lstlisting}
...
it as an inductive type and call the constructor $\mathtt{Exists}$.
\begin{lstlisting}
...
it to a value.  So the destructor is $\mathtt{Apply}$.
...
dependent case, the type checker wouldn't allow such a call because $\mathtt{hd}$\;and
...
$\mathtt{Maybe\langle\;A\;:\;Set\rangle}$\;\linebreak where $\mathtt{A}$\;can be an arbitrary type
of kind $\mathtt{Set}$.
...
\chapter{Type action proof}
\label{sec:org7541cdb}
