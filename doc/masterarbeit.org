...
* Introduction
  ...
  are defined In section [[Abstract Syntax]] I will develop the abstract syntax of
...
* Coinductive Types
  ...
  recursive record. In haskell we could never instantiate such a type, because
  too do this we already need something of type ~Stream A~ to fill in the field
  ~tail~. To remedy this the paper defines copattern matching. With the help of
  copattern matching we can define function which outputs expressions of type
  ~Stream A~. As an example we look at the difintion of repeat. This function
  takes in an a of type Nat and generates an stream which just infintly repeats
  it.
  ...
  As you can see copattern matching works via opservations i.e. we define
  what should be the output of the fields applied to the function.  This fields
  are also called observers, because we observe parts of the type.  Because
  inhapitants of ~Stream~ are infinitly long we can't print out a stream.  So
  we always have to use observers to show parts of it.  For example we can look
  at the third value of ~repeat 2~ via ~head (tail (tail (repeat 2)))~ which
  should evaluate to 2.  We can also implement a function which looks at the
  nth. value.  Here it is.
  ...
  Functions can also defined via a recursive record.  It is defined like the
  following.
  ...
  the following.  In fact we already used in the defintions of the functions
...
* Coniductive Types in dependent languages
  In this section i will look how coinductive types are implemented in dependently
  typed language.  In dependently typed languages types can depend on values.  This
  makes it neccessary to ensure that function terminate to make type checking decidable.
