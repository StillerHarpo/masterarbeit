...
*** (co)inductive types
    ...
    variables.  This is.
    \begin{prooftree}
    \AxiomC{$\Phi|\Theta|\Gamma'\vdash A_i:\Gamma_i$}
    \UnaryInfC{$\Phi|\Theta|\Gamma'\vdash X<\vv{A}> : \Gamma \rat *$}
    \end{prooftree}
    ...
    on the abstract syntax tree we also know $\Gamma$ and the $\Gamma_i$.
    $\Gamma$ is just the context from the definition and the $\Gamma_i$ are from
    the parameter context.  Because we already typed checked this definition we
    just have to check if types given for the parameters have the right type.
...
*** Expression variable introduction
    ...
    \begin{center}
      ...
      \RightLabel{\textbf{(Proj)}}
      \UnaryInfC{\graybox{$\Phi|\Theta$}$|\Theta|\Gamma\vdash x:A$}
      \DisplayProof
    \end{center}
...
*** Constructor
    ...
    gets rewritten to
    \begin{center}
      \AxiomC{\graybox{$\Sigma$(Constr)$\rightsquigarrow(\Gamma_k,y:A_k[\mu/X])\rat\mu@\sigma_k$}}
      \RightLabel{\textbf{(Ind-I)}}
      \UnaryInfC{\graybox{$\Sigma|\Theta|\Gamma$}$\vdash$Constr$:(\Gamma_k,y:A_k[\mu/X])\rat\mu@\sigma_k$}
      \DisplayProof
    \end{center}
...
*** Destructor
    ...
    gets rewritten to
    \begin{center}
      \AxiomC{\graybox{$\Sigma$(Destr)$\rightsquigarrow(\Gamma_k,y:\nu@\sigma_k)\rightarrow
        A_k[\nu/X]$}}
      \RightLabel{\textbf{(Ind-I)}}
      \UnaryInfC{\graybox{$\Sigma|\Theta|\Gamma$}$\vdash$Destr$:(\Gamma_k,y:\nu@\sigma_k)\rat
        A_k[\nu/X]$}
      \DisplayProof
    \end{center}
...
*** Recursion
    ...
    \begin{prooftree}
      \AxiomC{$\vdash C:\Gamma\rat*$}
      \AxiomC{\graybox{$\vv{\vdash B_k\equiv_\beta(C@\sigma_k)}$}}
      \AxiomC{\graybox{$\vv{\Sigma \vdash\text{Constr}_k:(\Gamma_k,y:A_k[\mu/X])\rat\mu@\sigma_k}$}}
      \noLine
      \UnaryInfC{$\vv{\Delta,\Gamma_k,y_k:A_k[C/X]\vdash g_k:\text{\graybox{$B_k$}}}$}
      \RightLabel{\textbf{(Ind-E)}}
      \TrinaryInfC{\graybox{$\Sigma|\Theta|$}$\Delta\vdash$ rec \graybox{$\mu$ to C};
...
*** Corecursion
    ...
    \begin{prooftree}
      \AxiomC{$\vdash C:\Gamma\rat*$}
      \AxiomC{\graybox{$\vv{\vdash B_k\equiv_\beta A_k[C/X]}$}}
      \AxiomC{\graybox{$\vv{\Sigma \vdash\text{Destr}_k:(\Gamma_k,y:\nu@\sigma_k)\rat
        A_k[\nu/X]}$}}
      \noLine
      \UnaryInfC{$\vv{\Delta,\Gamma_k,y_k:(\Gamma_k,y:(C@\sigma_k))\vdash g_k:\text{\graybox{$B_k$}}}$}
      \RightLabel{\textbf{(Coind-I)}}
      \TrinaryInfC{\graybox{$\Sigma|\Theta|$}$\Delta\vdash$ rec \graybox{C to $\nu$};
