...
** Abstract Syntax
   ...
   constructors/destructors will also be given names. In cite:basold2016type
...
*** Declarations
    ...
    respectively. After that we will write the name.  We can only use names
    which aren't used already. Behind that we can give a
    parameter context. This is a type context. This types are not polymorphic.
    They are merely macros to make the code more readable and allow the
    definition of nested types. If we want to use
    this type we have to fully instantiate this context. This types can occur
    ...
    will also give names to every constructor and destructor. This names have to
    be unique. Constructors and destructors also have contexts. Additionally
    ...
    \begin{figure}
     \begin{align*}
       \begin{array}{llll}
        ...
        ExprVar &:= &x,y,z,\dots &\text{Expression variables} \\
        TypeVar &:= &X,Y,Z,\dots &\text{Type expression} \\
             & &&          \text{variables}\\
        ParVar  &:= &A,B,C,\dots &\text{Parameter variables} \\
        ExprCtx &:= &\text{(} (ExprVar \text{ : } TypeVar)* \text{)} &\text{Expression Context} \\
        ParCtx &:= &\text{<} (ParVar \text{ : } ExprCtx \rightarrow \text{ Set})* \text{>} &\text{Parameter Context} \\
        Decl &:= &\text{data } N\; ParCtx \text{ : } ExprCtx \rightarrow \text{ Set where} &\text{Declarations}\\
                  &&\quad(N \text{ : }  ExprCtx \rightarrow\text{} TypeExpr \rightarrow N\; Expr*)* &\\
             &| &\text{codata } N\; ParCtx \text{ : } ExprCtx \rightarrow\text{ Set where}& \\
         &&\quad(N \text{ : }  ExprCtx \rightarrow\text{} N\; Expr* \rightarrow\text{} TypeExpr)*& \\
        &| &n \; ParCtx \; ExprCtx \text{ = } Expr & \\
      \end{array}
    \end{align*}
    ...
    + The other $N$ will be used later for
      ...
      $\xi_1^{\mu(X:\Gamma\rat *;\vv\sigma;\vv A)},\xi_2^{\mu(X:\Gamma\rat *;\vv\sigma;\vv A)},\dots$
      in the coinductive case
    + The $TypExpr$ are the $\vv{A}$
    + The $Expr*$ are the $\vv{\sigma}$
    + The first $ExprCtx$ is $\Gamma$
    + The other $ExprCtx$ stand for $\Gamma_1,\dots,\Gamma_m$
...
*** Expressions
    ...
    \begin{figure}
     \begin{align*}
       \begin{array}{llll}
         ...
         &| &ExprVar&\\
         &| &n\; ParInst\; ExprInst&\\
         Match &:= &N\; ExprVar* = Expr &\text{match}\\
         TypeExpr &:= &\text{(}ExprVar\text{ : }TypeExpr\text{).}TypeExpr &\text{Type expressions}\\
         &| &TypeExpr\text{ @ }Expr&\\
         &| &\text{Unit} &\\
         &| &TypeVar&\\
         &| &N\; ParInst? &\\
    ...
    application ~@~,the only primitive unit expression ~()~ or an variable.
    With the keyword ~rec~ we can destruct an inductive type. We write ~N
    ParInst? to TypeExrp~, where ~N~ is an previously defined inductive type and
    ~ParInst?~ the instantiation of its parameter context, after ~rec~ to
    facilitate type checking. It says we want to destruct an inductive type to
    some other type . We have to list all the constructors above one another. For
    each constructor we write an expression behind the equal sign, which should
    be of type ~TypeExpr~ which we have given above. In this expression we can
    use variables given in the match expression. The last one is the recursive
    occurrence. With the keyword ~corec~ we can do the same thing to construct a
    coinductive type. Here we have to swap the ~T~ and the ~TypeExpr~ and list
    the destructors. All con/destructors have to be instantiate with all
    variables in the parameter contexts of their types. This is done by giving
    types of the expected kinds separated by ',' enclosed in '<' and '>'. The
    variables are separated in local variables and global variables. Global
...
** Natural Numbers and Extended Naturals
   ...
   natural numbers with an additional value, infinty. We define it coinductively
   with the predecessor as it only destructor. The predecessor is either not
   ...
   ~x~ is just ~x~. So the only case of corec returns a ~Right x~ (remember Prec
   returns a coproduct not a number).
   #+begin_example
...
** Vectors and Streams
   ...
   Vectors are a standard example for dependent types. There are like lists,
   except there type depends on there length. For example a vector ~[1;2]~ has
   type ~Vector<Nat> 2~, because it length is 2. It has 2 constructors ~Nil~ and
   ...
   With the help extended naturals , we can define partial streams.  This are streams
