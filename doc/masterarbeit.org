...
* Coinductive Types in dependent languages
  ...
  function ~f : Nat \rat Nat~ and we want to check a value ~a~ against a type ~Vec
...
*** Expressions
    ...
    application ~@~,the only primitive unit expression ~\lozenge~ or an variable. With
    ...
    giving types of the expected kinds separated by ',' enclosed in ~\langle~ and ~\rangle~.
...
** Natural Numbers and Extended Naturals
   ...
   succ = corec Conat to Conat where
            Prec x $\rat$ Just$\langle$Conat$\rangle$ @ x
   #+end_example
...
** Binary Product and Coproduct
   ...
   \AxiomC{$(A : *, B : *)||(x:A) \vdash$ Snd @ x : Product$\langle$A,B$\rangle$ \textcircled{a}}
    \noLine
    \UnaryInfC{$(A : *, B : *)||(y : B) \vdash$ Fst @ y : Product$\langle$A,B$\rangle$ \textcircled{b}}
...
** Vectors and Streams
   ...
   type ~Vector\langle Nat\rangle  2~, because its length is 2. It has 2 constructors ~Nil~ and
