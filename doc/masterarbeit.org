...
*** Termination Checking with Sized Types
 ...
 of
    types allow us to annote data with their size.  Functions can use this
...
*** Statements
    ...
    respectively. After that we will write the name.  Behind that we can give a parameter context,
    This is a type context.  This types can occur everywhere in the definition where a type is expected.
    A (co)inductive Type can
    ...
    back a value of the type, where its context is instatiated. This corresponds
    to the sigmas in the paper. If we write a name before a equal sign we can
    bind the following expression to the name.  Every such defined name can depend
    on a parameter context and an argument context.  We write the parameter context
    like in the case for data types behind the name.  After that we can give a term
    context in between round parenthesis.
    \begin{lstlisting}
    statement =
