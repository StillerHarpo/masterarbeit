...
* Coinductive Types
  ...
  ~Stream A~. As an example we look at the definition of repeat. This function
  takes in an a of type Nat and generates an stream which just infinitely repeats
  it.
  ...
  As you can see copattern matching works via observations i.e. we define
  ...
  inhabitants of ~Stream~ are infinitely long we can't print out a stream.  So
