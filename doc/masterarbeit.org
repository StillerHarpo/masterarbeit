...
** Coinductive Types in Coq
   ...
   described in [[Postive Coinductive Types]].  It works over constructors.
   Therefore they are called postive types.  The newer and recommended one is
   described in section [[Negative Coinductive Types]]. They are defined
   over primitive records (a relatively new feature of coq).  Therefore they are
   called negative Types.

*** Postive Coinductive Types
   Positve coinductive types are defnined over constructors in coq.  The keyword
   CoInducvive is used to indicate that we about to define a coinductive type.
...
*** Negative Coinductive Types
    ...
    If we want to access parts of a stream we use the destructrors ~hd~ and
    ...
    i.e. we apply the value to the function.  To define a function we use
    ...
    accepted.  In this case, there is no recursive call.
...
** Coinductive Types in Agda
   ...
   terminate but are rejected by the type checker.  In fact there have to be
   such function because of the halting problem.  To allow more functions we can
   use a unique feature of agda, sized types.  They are described in section
...
*** Negative Coinductive Types in Agda
    ...
    repeat ~x~ infinitly ofen.  So every element of the steam is ~x~, including
    ...
    Infinitly often repated ~x~ is the same as x and then infinitly repeated
   ...
   Here we just pattern macht on the first argument (exluding the implicit
