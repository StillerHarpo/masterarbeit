...
* Introduction
  ...
  output. This functions don't depend on multiple value i.e. if they is no IO
  involved they produce for the same input always the same output. For example
  if we call a function or on true and false we always get true. This makes the
  code more predictable.

  The or function should only be working on numbers. To call it on strings
  "fu" and "bar", wouldn't make sense i.e. there is no defined output for this
  inputs. To prevent calls like this some functional programming languages
  introduced types.  Types contain only certain values.  For example the type
  bool contains only the values for true and false.  In Haskell we can define it
  like the following.
  #+begin_src haskell
  data Bool = True | False
  #+end_src
  This says we can construct values of type ~Bool~ with the constructors ~True~
  and ~False~. This which have constructors are called inductive types. We can
  ...
  or _    _    = False
  #+end_src
  Here we just list equations which say what the output for an given input is.
  ...
  ~Bool~.  Haskell won't compile our code.  Instead it gives back an type error.

  If we now want to change ~Bool~ to a three-valued logic, we have to add an third
  constructor to ~Bool~.  After that we to change every function which pattern
  matches on ~Bool~.  If we have many that would be a lot of repetitive work.
  If haskell would have coinductive type this could be a lot less work.
  Coinductive types are types which are, in contrary to inductive types defined
  over their destruction.  Se we could define ~Bool~ over its destructors.  This
  would be or,and, etc.
...
** Typing rules
   ...
   *TyCtx*.
   ...
   + $A \equiv B$ - The type $A$ is computational equivalent to type $B$
...
*** Context rules
    ...
    \DisplayProof
    \hskip 1.5em
    \AxiomC{$\mid\emptyset\Gamma\vdash A:*$}
...
*** Beta-equivalence
    To type are beta equivalent if they evaluate to the same type. Because our
    ...
    alpha equivalent, then $A$ and $B$ are beta equivalent In the implementation
    $\equiv_\alpha$ is trivial, because we use /de Bruijn indices/.
...
*** Unit type and expression introduction
    ...
    second rule says its value $\lozenge$ is always of type $\top$. This rules
    get rewritten to.
    \begin{center}
      \AxiomC{}
      ...
      \hskip 1.5em
      \AxiomC{}
      \topI{\graybox{$\Phi\mid\Theta\mid\Gamma$}$\vdash$():Unit}
      \DisplayProof
    \end{center}
    We change the syntax "$\top$" to "Unit" and add *Ctx* and *TyCtx*. We will
    do this for every rule which has empty contexts to subsume the weakening
    rules of the paper. The unit term always has the unit type as its type.
...
*** Variable lookup
    ...
    context.  In our rules we just look up the variable in the context.  This
...
*** Type and expression instantiation
    ...
    \begin{center}
      ...
      \DisplayProof
    \end{center}
    This rules are similar to the rule for type instantiation.  Here we have to
...
*** Constructor and Destructor
    ...
    and an value of type $A_k[\mu/X]$, which is the type, which can contain the
...
*** Recursion and Corecursion
    ...
    \begin{prooftree}
     ...
     be destructed to. We get the syntax directed rule analog as in case of Recursion.
...
** Natural Numbers and Extended Naturals
   ...
   case of corec returns a ~Just x~ (remember Prec returns a ~Maybe<Conat>~ not
   a ~Contat~).
   #+begin_example
   ...
   zero = (corec Unit to Conat where
             {Prev x = nothing$\langle$Unit$\rangle$}) @ ()
   infinity = (corec Unit to Conat where
                 {Prev x = Just$\langle$Conat$\rangle$ @ ()}) @ ()
   #+end_example
   ...
   ~corec~ to ~()~ to get the value.  The predecessor of ~infinity~ should also
   be ~infinity~.  We apply the ~corec~ to another ~Conat~, so the ~x~ is also a
   ~Conat~.  We will no see that the predecessor on this values give back the
   right value.
   #+begin_example
     Prev @ zero $\succ \widehat{\text{Maybe}\langle X\rangle} \left(\underbrace{
        ...
        \end{subarray}}_{t_1}\right) @ x)$[nothing$\langle$Unit$\rangle$/x][()]
                 = rec Maybe$\langle$Unit$\rangle$ to Maybe$\langle$Conat$\rangle$ where
                     { Nothing u = Nothing$\langle$Conat$\rangle$ @ $\widehat{\text{Unit}}(t_1,x)$
                       Just c = Just$\langle$Conat$\rangle$ @ $\widehat{X}(t_1,x)$} @ x [nothing$\langle$Unit$\rangle$/x][()]
                 ...
                 $\succ$ Nothing$\langle$Conat$\rangle$ @ u[$\widehat{\text{Unit}}$(t_2 @ x)/u][()]
                 = Nothing$\langle$Conat$\rangle$ @ u[x/u][()]
                 = Nothing$\langle$Conat$\rangle$ @ ()
     Prev @ infinity $\succ \widehat{\text{Maybe}\langle X\rangle} \left(\underbrace{
        \begin{subarray}{l}
          \text{(corec Unit to Conat where}\\
          \quad\text{\{ Prec x = Just}\langle\Unit\rangle @ () \text{\}} @ x)
        \end{subarray}}_{t_1}\right) @ x)$[Just$\langle$Unit$\rangle$@()/x][()]
                     = rec Maybe$\langle$Unit$\rangle$ to Maybe$\langle$Conat$\rangle$ where
                         { Nothing u = Nothing$\langle$Conat$\rangle$ @ $\widehat{\text{Unit}}(t_1,x)$
                           Just x = Just$\langle$Conat$\rangle$ @ $\widehat{X}(t_1,x)$} @ x [Just$\langle$Unit$\rangle$@()/x][()]
                     = $\underbrace{\begin{subarray}{l}
                       \recT{\Maybe\langle\Unit\rangle}{\Maybe\langle\Conat\rangle} \\
                       \quad\text{\{} \Nothing\; u = \Nothing\langle\Conat\rangle @ u \\
                       \quad\;\Just\; x = \Just\langle\Conat\rangle @ t_1 \text{\}}
                      \end{subarray}}_{t_2}$ @ Just$\langle$Unit$\rangle$@()
                     $\succ$ Just$\langle$Conat$\rangle$ @ $t_1$[$\widehat{\text{Unit}}(t_2 @ x)$/x][()]
                     = Just$\langle$Conat$\rangle$ @ $t_1$[x/x][()]
...
** Vectors and Streams
   ...
   #+end_example
   This streams are like vectors except they also can be infinite long. This is
   ...
   ~tl~ expect streams which are at least of length one.
