...
* Typing rules
  ...
  We don't need the weaking rules because we can lookup a variable in a context.
  So the following rules get removed.
  + *(TyVar-Weak)*
  + *(Ty-Weak)*
  The order in *TyCtx* isn't relevant so we use a Map for it.  The order of *Ctx*
  is relevant because types of later variables can refer to former variables and application
  instantiate the first varibale in *Ctx*

  We also rewrite the rules which are already syntax-directed to rules which work on our syntax

  We add a new Ctx for data types

** Context rules
   \begin{prooftree}
   \AxiomC{\Delta|X_1:*,\dots,X_n:*|\Gamma\vdash A:*
   \UnaryInfC{\vdash\Gamma,x<X_1,\dots,X_n)
   \end{prooftree}
